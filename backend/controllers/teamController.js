...
exports.updateTeam = async (req, res) => {
  try {
    ...
    const { name, memberIds, tutor, classRef } = req.body; // Ajout de classRef
    
    // Si classRef est fourni (admin seulement)
    if (classRef && req.user.role === 'ADMIN') {
      const classExists = await Class.findById(classRef);
      if (!classExists) {
        return res.status(400).json({ message: 'Invalid class ID' });
      }
      team.classRef = classRef;
    }
    ...
    await team.populate('members.user', 'firstName lastName email');
    await team.populate('createdBy', 'firstName lastName email');
    await team.populate('classRef', 'name description department');
    await team.populate('tutor', 'firstName lastName email');
...
exports.getAllTeams = async (req, res) => {
  try {
    ...
    // Fetch all teams with full population of references
    const teams = await Team.find()
        .populate('members.user', 'firstName lastName email classe')
        .populate('createdBy', 'firstName lastName email classe')
        .populate({
          path: 'classRef',
    ...
    // Ensure every team has a valid classRef
    for (const team of teams) {
      // If classRef is missing but creator has a classe, use that
      if ((!team.classRef || !team.classRef.name) && team.createdBy && team.createdBy.classe) {
        try {
          const classInfo = await Class.findById(team.createdBy.classe).select('name description department');
          if (classInfo) {
            team.classRef = classInfo;
            await team.save();
          }
        } catch (err) {
          console.error(`Failed to update classRef for team ${team._id}:`, err);
        }
      }
    }
