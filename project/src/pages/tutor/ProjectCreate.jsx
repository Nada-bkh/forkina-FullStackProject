...
import { Box, Paper, Typography, TextField, Button, Grid, Chip, InputAdornment, IconButton, FormHelperText, Alert } from '@mui/material';
...
const ProjectCreate = ({ role = getUserRole() }) => {
  ...
  const [formData, setFormData] = useState({
    name: '', description: '', startDate: dayjs(), endDate: dayjs().add(30, 'day'), tags: []
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const projectData = {
        ...
        tags: formData.tags, status: 'PENDING'
      };
      const data = await createProject(projectData);
      setSuccess(true);
      setFormData({ name: '', description: '', startDate: dayjs(), endDate: dayjs().add(30, 'day'), tags: [] });
      setTimeout(() => {
        navigate(`${role === 'ADMIN' ? '/admin' : '/tutor'}/projects/${data._id}`);
      }, 1500);
    } catch (err) {
  ...
  return (
      <Box sx={{ p: 3 }}>
        ...
        {success && <Alert severity="success" sx={{ mb: 3 }}>Project created successfully! Redirecting...</Alert>}
        <Paper sx={{ p: 3, borderRadius: 2 }}>
          <form onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <TextField
                    fullWidth label="Add Tags" value={tagInput} onChange={handleTagInputChange} onKeyPress={handleKeyPress} disabled={loading}
                    InputProps={{
                      endAdornment: (
